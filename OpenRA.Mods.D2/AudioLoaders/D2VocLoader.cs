...
{
	public class D2VocLoader : ISoundLoader
	{
		bool ISoundLoader.TryParseSound(Stream stream, out ISoundFormat sound)
		{
			try
			{
				sound = new D2VocFormat(stream);
				return true;
			}
			catch
			{
	...
	public sealed class D2VocFormat : ISoundFormat
	{
		...
		public D2VocFormat(Stream stream)
		{
			this.stream = stream;
		...
		D2VocFormat(D2VocFormat cloneFrom)
		{
		...
		static void Preload(Stream stream, out D2VocBlock[] blocks, out int totalSamples, out int sampleRate)
		{
			var blockList = new List<D2VocBlock>();
			totalSamples = 0;
			sampleRate = 0;

			while (true)
			{
				D2VocBlock block = new D2VocBlock();
				try
				{
		...
		void Rewind()
		{
			currentBlock = ((IEnumerable<D2VocBlock>)blocks).GetEnumerator();
		...
		bool EndOfData
		{
			get
			{
				try
				{
					return currentBlock.Current.Equals(blocks.Last()) && samplesLeftInBlock == 0;
				}
				catch
				{
					return true;
		...
		public class D2VocStream : Stream
		{
			D2VocFormat format;
			public D2VocStream(D2VocFormat format)
			{
				this.format = format;
			}
